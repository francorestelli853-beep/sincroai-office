...
import { mockAgents, mockActivityLog } from '@/lib/mock-data'
...
const AGENT_AVATARS: Record<string, string> = Object.fromEntries(
  mockAgents.map((a) => [a.id, a.avatar])
)
...
// Group entries by day, preserving sort order
...
export default function ActivityPage() {
  ...
  }, [search, agentFilter, categoryFilter, dateFilter])
        ...
        <div className="rounded-full border border-gray-700 bg-gray-800 px-3 py-1.5">
          <span className="text-xs font-medium text-gray-300">
            ...
            <span className="text-muted-foreground">
              {filtered.length === mockActivityLog.length
                ? 'eventos totales'
                : `de ${mockActivityLog.length} eventos`}
            </span>
          </span>
        </div>
      </div>
